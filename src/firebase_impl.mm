...
DataSnapshotPtr toMojoSnapshot(FDataSnapshot* snapshot) {
  DataSnapshotPtr mojoSnapshot(DataSnapshot::New());
...
ErrorPtr toMojoError(NSError* error) {
  ErrorPtr mojoError(Error::New());
  mojoError->code = error.code;
  mojoError->message = error.description.UTF8String;
  return mojoError.Pass();
}

AuthDataPtr toMojoAuthData(FAuthData* authData) {
  AuthDataPtr mojoAuthData(AuthData::New());
...
FirebaseImpl::FirebaseImpl(mojo::InterfaceRequest<Firebase> request)
...
void FirebaseImpl::InitWithUrl(const mojo::String& url) {
  client_ = [[ ::Firebase alloc] initWithUrl:@(url.data())];
}

void FirebaseImpl::AddValueEventListener(ValueEventListenerPtr ptr) {
  ValueEventListener* listener = ptr.get();
  ...
  ptr.set_connection_error_handler([this, handle, listener]() {
    ...
    auto it = std::find_if(value_event_listeners_.begin(),
                           ...
                           [listener](const ValueEventListenerPtr& p) {
                             return (p.get() == listener);
                           });
    assert(it != value_event_listeners_.end());
    value_event_listeners_.erase(it);
  });
  value_event_listeners_.emplace_back(ptr.Pass());
}

void FirebaseImpl::AddChildEventListener(ChildEventListenerPtr ptr) {
  ChildEventListener* listener = ptr.get();
  void (^cancelBlock)(NSError*) = ^(NSError* error) {
    listener->OnCancelled(toMojoError(error));
  };

  void (^addedBlock)(FDataSnapshot*, NSString*) =
      ^(FDataSnapshot* snapshot, NSString* prevKey) {
  ...
  void (^changedBlock)(FDataSnapshot*, NSString*) =
      ^(FDataSnapshot* snapshot, NSString* prevKey) {
        listener->OnChildChanged(toMojoSnapshot(snapshot), prevKey.UTF8String);
      };
  FirebaseHandle changedHandle =
      [client_ observeEventType:FEventTypeChildChanged
          andPreviousSiblingKeyWithBlock:changedBlock
                         withCancelBlock:cancelBlock];

  void (^movedBlock)(FDataSnapshot*, NSString*) =
      ^(FDataSnapshot* snapshot, NSString* prevKey) {
  ...
  FirebaseHandle removedHandle =
      [client_ observeEventType:FEventTypeChildRemoved
                      withBlock:removedBlock
                withCancelBlock:cancelBlock];

  ptr.set_connection_error_handler([this, addedHandle, changedHandle,
                                    movedHandle, removedHandle, listener]() {
    ...
    auto it = std::find_if(child_event_listeners_.begin(),
                           ...
                           [listener](const ChildEventListenerPtr& p) {
                             return (p.get() == listener);
                           });
    assert(it != child_event_listeners_.end());
...
void FirebaseImpl::AuthAnonymously(const AuthAnonymouslyCallback& callback) {
  AuthAnonymouslyCallback* copyCallback = new AuthAnonymouslyCallback(callback);
  [client_ authAnonymouslyWithCompletionBlock:^(NSError* error,
                                                FAuthData* authData) {
...
void FirebaseImpl::AuthWithPassword(const mojo::String& email,
...
void FirebaseImpl::GetChild(const mojo::String& path,
...
void FirebaseImpl::GetRoot(mojo::InterfaceRequest<Firebase> request) {
...
void FirebaseImpl::SetValue(const mojo::String& jsonValue,
  ...
  SetValueCallback* copyCallback = new SetValueCallback(callback);
  ...
  void (^completionBlock)(NSError*, ::Firebase* ref) =
      ^(NSError* error, ::Firebase* ref) {
...
void FirebaseImpl::RemoveValue(const RemoveValueCallback& callback) {
  RemoveValueCallback* copyCallback = new RemoveValueCallback(callback);
...
void FirebaseImpl::SetPriority(int32_t priority,
  ...
  SetPriorityCallback* copyCallback = new SetPriorityCallback(callback);
...
void FirebaseImpl::CreateUser(const mojo::String& email,
  ...
  CreateUserCallback* copyCallback = new CreateUserCallback(callback);
...
void FirebaseImpl::ChangeEmail(const mojo::String& oldEmail,
  ...
  ChangeEmailCallback* copyCallback = new ChangeEmailCallback(callback);
...
void FirebaseImpl::ChangePassword(const mojo::String& newPassword,
                                  const mojo::String& email,
                                  const mojo::String& oldPassword,
                                  const ChangePasswordCallback& callback) {
  ChangePasswordCallback* copyCallback = new ChangePasswordCallback(callback);
...
void FirebaseImpl::RemoveUser(const mojo::String& email,
  ...
  RemoveUserCallback* copyCallback = new RemoveUserCallback(callback);
...
void FirebaseImpl::ResetPassword(const mojo::String& email,
  ...
  ResetPasswordCallback* copyCallback = new ResetPasswordCallback(callback);
...
void FirebaseFactory::Create(mojo::ApplicationConnection* connection,
                             mojo::InterfaceRequest<Firebase> request) {
